...
TEST_CASE("handler") {
    actors_framework::supervisor supervisor_(new dummy_supervisor(1, 100));

    REQUIRE(dummy_supervisor::constructor_counter == 1);

    actors_framework::send(supervisor_, actors_framework::address_t::empty_address(), "create_test_handlers");
    ...
    actors_framework::send(supervisor_, actors_framework::address_t::empty_address(), "create_storage");
