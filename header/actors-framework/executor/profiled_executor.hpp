...
namespace actors_framework::executor {
    ...
    public:
        ...
        using actor_id = typename profiled<work_sharing>::actor_id;
        ...
        public:
            ...
            auto operator-(const measurement& other) -> measurement&;
            auto operator=(const measurement& other) -> measurement& = default;
    ...
    auto profiled_executor<>::measurement::operator+(const measurement& other) -> measurement& {
    ...
    profiled_executor<>::profiled_executor(std::size_t num_worker_threads, std::size_t max_throughput)
    ...
    void profiled_executor<>::start_measuring(size_t worker, actor_id job) {
        auto& w = worker_states_[worker];
        w.current = job;
        w.job = measurement::take();
    }

    void profiled_executor<>::stop_measuring(size_t worker, actor_id job) {
    ...
    void profiled_executor<>::report(const actor_id& job, const measurement& m) {
        std::lock_guard<std::mutex> job_guard{job_mtx_};
        jobs_[job] += m;
    ...
    template<class Time, class Label>
    void profiled_executor<>::record(Time t, Label label, size_t rec_id, const profiled_executor<>::measurement& m) {
